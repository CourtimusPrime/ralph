---
name: nest
description: "Break a feature description into multiple scoped sub-PRD markdown files for use with Ralph's nested mode. Use when the user wants to split a large feature across multiple focused PRD files. Triggers on: /nest, nested prd, split into prds, break this into prds."
user-invocable: true
---

# Nest — Multi-PRD Generator

Break a feature description into multiple focused sub-PRD markdown files that work together as a nested set. Each file covers one domain of the feature. All files share a master config (`prd-master.json`) and are later compiled into a single `prd.json` by `merge-prds.sh`.
...
7. Write each `tasks/nest/[feature-slug]/prd-NN-[scope].md` sub-PRD

**Do NOT create `prd-master.json`.** Project metadata (project name, branchName, description) is inferred automatically by `/ralph` when it reads the folder.
...
```
I'll split this into 3 sub-PRD files:
...
Each nest run gets its own subdirectory under `tasks/nest/` named after the feature slug. Sub-PRD files use numeric prefixes to control merge order:

```
tasks/nest/[feature-slug]/prd-master.json
tasks/nest/[feature-slug]/prd-01-[scope].md
tasks/nest/[feature-slug]/prd-02-[scope].md
tasks/nest/[feature-slug]/prd-03-[scope].md
```
...
```
Created 3 sub-PRD files in tasks/nest/budget-tracker/:
...
Next steps:
  1. Review and edit the files in tasks/nest/budget-tracker/
  2. Run: /ralph tasks/nest/budget-tracker   (compiles into scripts/ralph/prd.json)
  3. Run: ./scripts/ralph/ralph.sh
```

Each `/nest` run produces an isolated folder, so multiple features can coexist in `tasks/nest/` without any file collisions. No `prd-master.json` is needed — `/ralph` infers project metadata from the folder name and `package.json`.
...
- [ ] No `prd-master.json` created (deprecated — `/ralph` infers metadata)
