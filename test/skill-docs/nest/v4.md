...
Break a feature description into multiple focused sub-PRD markdown files that work together as a nested set. Each file covers one domain of the feature. Files are later compiled into `scripts/ralph/prd.json` by running `/ralph tasks/[feature-slug]`.
...
Each nest run gets its own subdirectory directly under `tasks/` named after the feature slug. Sub-PRD files use numeric prefixes to control merge order:
...
Each `/nest` run produces an isolated folder directly in `tasks/`, so multiple features can coexist without collisions. No `prd-master.json` is needed — `/ralph` infers project metadata from the folder name and `package.json`.
...
- [ ] Files written to `tasks/[feature-slug]/` (not `tasks/nest/[feature-slug]/`)
