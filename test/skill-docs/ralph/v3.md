...
6. Determine nest output paths: `scripts/ralph/nests/[slug]/prd-NN-[scope].json` where `[slug]` is the folder's basename
...
8. Write each file's stories to its corresponding nest sub-file (`scripts/ralph/nests/[slug]/prd-NN-[scope].json`)
9. Write `scripts/ralph/prd.json` as a **manifest only** — `nests` array listing all sub-file paths, no `userStories` key
