...
6. Determine nest output paths — for each source file, map it to a sub-file path:
   - Source: `tasks/budget-tracker/prd-01-data-layer.md`
   - Output: `scripts/ralph/nests/budget-tracker/prd-01-data-layer.json`
   - Pattern: `scripts/ralph/nests/[slug]/[source-filename-stem].json`
   - `[slug]` = folder basename (e.g., `budget-tracker`)
   - Filename preserves the `prd-NN-[scope]` name from the source file — do NOT prepend the slug to the filename
...
8. Write each file's stories to its corresponding nest sub-file. Create the `scripts/ralph/nests/[slug]/` directory if it does not exist.
