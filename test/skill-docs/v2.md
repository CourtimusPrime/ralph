...
7. Write `tasks/nest/[feature-slug]/prd-master.json`
8. Write each `tasks/nest/[feature-slug]/prd-NN-[scope].md` sub-PRD
...
## Step 4: Write prd-master.json

Create `tasks/nest/[feature-slug]/prd-master.json` with:

```json
{
  "project": "[Project name inferred from feature or existing package.json]",
...
Created 3 sub-PRD files in tasks/nest/budget-tracker/:
  ...
  prd-master.json              project config
...
Next steps:
  ...
  2. Run: ./scripts/ralph/merge-prds.sh --tasks-dir tasks/nest/budget-tracker/
  3. Run: ./scripts/ralph/ralph.sh
```

The `--tasks-dir` flag points `merge-prds.sh` at the feature's subdirectory. Each `/nest` run produces an isolated folder, so multiple features can coexist in `tasks/nest/` without any file collisions.
...
- [ ] `prd-master.json` written to `tasks/nest/[feature-slug]/`
